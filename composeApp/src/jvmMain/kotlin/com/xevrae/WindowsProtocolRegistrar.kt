...
/**
 * Registers the "xevrae://" custom URI protocol handler in Windows Registry
 * under HKEY_CURRENT_USER (no admin rights required).
 *
 * Registry structure:
 * ```
 * HKCU\Software\Classes\xevrae
 *     (Default) = "URL:Xevrae Protocol"
 *     URL Protocol = ""
 *     \DefaultIcon
 *         (Default) = "\"<exe_path>\",0"
 *     \shell\open\command
...
object WindowsProtocolRegistrar {
    ...
    private const val SCHEME = "xevrae"
    ...
    fun register() {
        ...
        try {
            ...
            Logger.d(TAG, "Registering xevrae:// protocol handler -> $exePath")

            // Main key with protocol description
            regAdd(REG_KEY, null, "URL:Xevrae Protocol")
    ...
    private fun isAlreadyRegistered(currentExePath: String): Boolean {
        return try {
            ...
            // Registry stores path with quotes: "C:\path\to\Xevrae.exe" "%1"
    ...
    private fun resolveExePath(): String? {
        ...
        //   <app>/Xevrae.exe
        // So we go: java.home → parent (runtime) → parent (app) → Xevrae.exe
        ...
        if (appDir != null) {
            val exeFile = java.io.File(appDir, "Xevrae.exe")
            if (exeFile.exists()) {
