...
kotlin {
    ...
    android {
        namespace = "com.xevrae.composeapp"
        compileSdk = 36
        minSdk = 26
        withJava()
        androidResources {
                ...
                val macExtraPlistKeys =
                    ...
                    <array>
                        <dict>
                            ...
                            <string>com.xevrae.deeplink</string>
                            <key>CFBundleURLSchemes</key>
                            <array>
                                <string>xevrae</string>
                            </array>
                        </dict>
                    </array>
                    """.trimIndent()
...
afterEvaluate {
    ...
    fun packAppImage(isRelease: Boolean) {
        ...
        // Generate xevrae.desktop dynamically
        val versionName =
            libs.versions.version.name
                .get()
        val desktopFile = appDir.resolve("xevrae.desktop")
        desktopFile.writeText(
            """[Desktop Entry]
            |Type=Application
            |Version=1.0
            |Name=Xevrae
            |Comment=Xevrae v$versionName - FOSS YouTube Music Client
            |Exec=bin/Xevrae %u
            |Icon=xevrae
            |Terminal=false
            |Categories=Audio;AudioVideo;
            |StartupWMClass=com--xevrae-MainKt
            |MimeType=x-scheme-handler/xevrae;
            |
            """.trimMargin(),
        )
        ...
        appRun.writeText(
            ...
            |if [ ! -f "${'$'}ICON_DIR/xevrae.png" ] || [ "${'$'}HERE/xevrae.png" -nt "${'$'}ICON_DIR/xevrae.png" ]; then
            |    mkdir -p "${'$'}ICON_DIR"
            |    cp "${'$'}HERE/xevrae.png" "${'$'}ICON_DIR/xevrae.png"
            ...
            |sed "s|Exec=bin/Xevrae|Exec=${'$'}APPIMAGE_PATH|" "${'$'}HERE/xevrae.desktop" > "${'$'}DESKTOP_DIR/com--xevrae-MainKt.desktop"
